...
**Server-block layout** (precedence: exact match > regex; ties go in definition order):

| Pattern | Source | Matches |
|---------|--------|---------|
| `server_name ${BASE_DOMAIN}` (exact) | static (`nginx-base.conf.template`) | Frontend SPA + `/api/` + optional `/git/` |
| `server_name <alias>.${BASE_DOMAIN}` (exact, per alias) | dynamic (`/etc/nginx/dynamic/aliases.conf`, auto-generated by `aliasesService`) | Custom workspace subdomain alias → `workspace-<uuid>:<port>`, optionally `auth_basic` |
| `server_name ~^[a-f0-9-]+-[0-9]{1,5}\.${BASE_DOMAIN}$` (regex) | static | Workspace port subdomain (default URL) → `workspace-<uuid>:<port>` |
| `server_name ~^[a-f0-9-]+\.${BASE_DOMAIN}$` (regex) | static | code-server IDE → `workspace-<uuid>:8082` |
| `server_name _; default_server` | static | Catch-all → `HTTP 404 "Unknown subdomain..."` |

The dynamic alias block is the reason exact-match aliases override the regex-based UUID routing; the catch-all is the reason unknown subdomains don't fall through to the (potentially auth-protected) frontend.

**Frontend / API location blocks** (inside the `${BASE_DOMAIN}` server):
```nginx
location / {
...
The dynamic alias config is regenerated by the server (`aliasesService.generateNginxConfig()`) and applied via `docker exec xaresaicoder-nginx nginx -t && nginx -s reload`. On `nginx -t` failure the previous config is rolled back from `last-known-good.txt`.
