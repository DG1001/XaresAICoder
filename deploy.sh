...
deploy_application() {
    ...
    if [ "$git_server_enabled" = "true" ]; then
        # Check if FORGEJO_ROOT_URL is already set in .env (for external SSL proxy setups)
        local forgejo_root_url_override=$(grep "^FORGEJO_ROOT_URL=" .env | cut -d'=' -f2 | tr -d ' ')

        if [ -n "$forgejo_root_url_override" ]; then
            # Use the override from .env (e.g., for external SSL proxy)
            export FORGEJO_ROOT_URL="$forgejo_root_url_override"
            print_status "Forgejo ROOT_URL: $forgejo_root_url_override (from .env override)"
        else
